+++
title = 'Finite Difference in Matlab有限差分与最速降线的矩阵化求解'
date = 2026-03-31T14:55:43+08:00
draft = false
mathkatex = true
categories = ['matlab']
tags = ['matlab', 'finite difference', 'brachistochrone', 'numerical methods']
toc = true
tocBorder = true
+++


![最速降线问题示意图](/matlab/pde/figures/brachistochrone_compare.gif)
...
在等距网格 $x_i=x_{\min}+(i-1)h$ 上，设 $y_i=f(x_i)$，目标是构造一次性作用于全区间的离散导数算子。对任意模板点位移集合 $s_j$，令局部近似写成 $f'(x_i)\approx \sum_{j=1}^{5}k_j f(x_i+s_j h)$，将五个采样点做泰勒展开并要求常数项、二至四阶项消失，一阶项系数为 1，就得到与 [cd5p_deduction.m](/matlab/pde/cd5p_deduction.m) 相同的线性系统求解过程。
...
```matlab
{{% codeseg "static/matlab/pde/cd5p_deduction.m"  %}}
```

每一节都可以得到一组系数，作为有限差分的模板。对于内部点，选取对称模板 $s=[-2,-1,0,1,2]$ 得到五点中心差分模板 $[1/12,-2/3,0,2/3,-1/12]$；对于边界点，选取与网格左端或右端对齐的单边模板，如 $s=[0,1,2,3,4]$ 或 $s=[-1,0,1,2,3]$，得到前向或后向四阶差分模板。

![五点差分网格](/matlab/pde/figures/five_point_stencil_1d.png)

接下来就要构成一个矩阵来一次性作用于全区间的离散函数值向量 $\mathbf y$。左端第一个点使用 $s=[0,1,2,3,4]$ 时得到前向四阶模板 $[-25/12,4,-3,4/3,-1/4]$，第二个点使用 $s=[-1,0,1,2,3]$ 时得到 $[-1/4,-5/6,3/2,-1/2,1/12]$，内部点使用 $s=[-2,-1,0,1,2]$ 时得到中心模板 $[1/12,-2/3,0,2/3,-1/12]$，右端两点由镜像模板给出。将这些系数按行装配，便得到五对角带状矩阵 $A$，从而全局导数可写为
...
![五点四阶差分的导数结果](/matlab/pde/figures/fd_derivative.png)

![五点四阶差分矩阵的结构](/matlab/pde/figures/fd_matrix.png)
...
$$
每一步都只需求解一个线性方程组，然后做阻尼更新 $\mathbf u^{(k+1)}=\mathbf u^{(k)}+\lambda\Delta\mathbf u$，并通过减小 $\lambda$ 保证 $y>0$。这就是“最速降线差分问题转化为线性方程组迭代求解”的核心机制。当前代码 [solve_brachistochrone_fd.m](/matlab/pde/solve_brachistochrone_fd.m) 已按该矩阵化思路实现，[demo_fd_and_brachistochrone.m](/matlab/pde/demo_fd_and_brachistochrone.m) 给出图像与残差检验，典型算例在 5 步左右收敛到 $10^{-11}$ 量级残差。



![最速降线边值问题的差分-线性化求解结果](/matlab/pde/figures/brachistochrone_solution.png)
...
![最速降线数值解与摆线理论解对比](/matlab/pde/figures/brachistochrone_analytic_compare.png)
